...
3. **Admin & SEO field names are standardized** to a single fixed format (see §1), since golden-example files have historically disagreed with each other on field naming. Writer output must use exactly: `Title, Pitch, Slug, Tags, Meta Title (≤60 chars), Meta Tag Keywords, Target Audience, Content Type, Difficulty Level, Author, Categories, Related Products, Related Tutorials, Publish Date` as `| Field | Draft Value |` table rows — no `Revamp Status`/`Validity`/`Decision`/`Priority` row (those are internal dashboard fields, not CMS/SEO metadata). **`Meta Tag Keywords` (Milestone 8, human-approved) replaces the older `Meta Description` field** — never produce both, and never produce `Meta Description` at all. Its value is a concise comma-separated keyword list (e.g. `ESP32, Maker ESP32, LED Pattern, Arduino, GPIO, Beginner`), sourced deterministically from this tutorial's own existing keywords/tags — not a free sentence, and not a fabricated long SEO list. `Meta Title`'s own 60-character rule is unchanged.
...
## 28. Maker ESP32 Global Rules & Publish-Status Lifecycle (Milestone 7)

*Human-approved rules, generalized. The actual canonical URLs live in code (`service/approvedLinks.js`), never duplicated here so there is one source of truth; the actual publish-date schedule lives in `service/publishSchedule.js`, deliberately kept OUT of this file since it is injected verbatim into every writer prompt — a growing project-history schedule table does not belong in a prompt every job pays the token cost of.*

1. **Canonical Maker ESP32 / Getting Started / Telegram links are pre-approved, globally.** A fixed set of URLs (the Maker ESP32 product page, the Maker ESP32 Getting Started guide, the ESP32 Makers Telegram community, and the two Maker Port cable products — see `service/approvedLinks.js`) are human-approved and trusted for every tutorial. They are never searched for, never rewritten, and never marked `NEEDS VERIFICATION`.
2. **Maker ESP32 product-migration default.** When a tutorial is migrated from NodeMCU ESP32 to Maker ESP32, the new controller is standalone Maker ESP32 — do not retain NodeMCU ESP32 in the final architecture and do not automatically introduce Robo ESP32, unless the human explicitly says otherwise for that job. A `PROJECT_HARDWARE_DECISIONS` entry (§25) may confirm or override this for one specific tutorial.
3. **Maker ESP32 public-linking rule.** Any visible public tutorial text reading "Maker ESP32" must hyperlink that text to the canonical product page — consistently across headings, introductory paragraphs, the BOM, wiring explanations, software instructions, testing, troubleshooting, and related products — without double-wrapping an already-linked occurrence. This never applies inside code blocks/inline code, raw URLs, HTML attributes, or Internal Editor Notes — those are not public prose. A deterministic check (`draftValidator.js`'s `maker_esp32_public_links`) enforces this on the writer's own output; it deliberately does not scan the Admin & SEO metadata table's Tags/Meta Title/Meta Tag Keywords cells (CMS metadata, not rendered prose) or editorial placeholder markers (`[EDITOR PLACEHOLDER: ...]` / `[INTERNAL MEDIA PLACEHOLDER: ...]`).
4. **Maker Port cable selection is evidence-based, never guessed.** "Use the Maker Port on Maker ESP32" means preferring the JST-SH/Qwiic-style Maker Port connection over manual GPIO header wiring — but only when the component/interface is genuinely compatible (see §23.5 and the HARDWARE COMPATIBILITY CONTRADICTION RULE). When Maker Port is the right approach: a component needing female-socket header connections uses the STEMMA QT/Qwiic JST-SH-to-female-sockets cable; a component with a Grove port uses the Grove-to-JST-SH(Qwiic) cable. Apply the standard EVIDENCE & DECISION PRIORITY order (§27) to determine which case applies — absence from the Maker ESP32 Coding Pack alone is not sufficient to call a component's interface unresolved. If the interface is genuinely not established by any source, do not invent the connector; record it under Outstanding Verification instead.
5. **Getting Started guide is a known, always-linkable URL.** Since the canonical Getting Started guide URL is now known, Prerequisites for a Maker ESP32 tutorial always links it directly — never marked `NEEDS VERIFICATION` for that reason again. This does not change §23.4's rule that Prerequisites stays minimal and Software Setup does not repeat generic setup steps already covered by that guide.
6. **Telegram community link is text, never an image, unless a real banner asset exists.** The canonical ESP32 Makers Telegram URL is a destination link, not an image asset — never write `![...](https://t.me/...)`. Use a normal Markdown text link (e.g. `[ESP32 Makers Community](...)`) unless an approved banner IMAGE URL is separately available.
7. **Human-approved publish-date authority.** When a tutorial has a human-approved scheduled publish date on record, that date is authoritative for the Admin & SEO "Publish Date" field — never today's date, the job creation date, the generation date, or the revision date. A revision must preserve or correct this date if an earlier draft got it wrong; the writer never silently patches an already-approved candidate during publish — a wrong date is caught by validation (`scheduled_publish_date_consistency`, a hard FAIL on mismatch) before a human ever approves it. The scheduled date is stored internally as ISO (`service/publishSchedule.js`, e.g. `2026-09-14`) — that internal storage form is correct and unchanged for sorting/machine comparison. The **public** Admin & SEO "Publish Date" value (and any other tutorial-facing publish-date text) must NEVER show that ISO form; it must show the human-facing `D MMM YYYY` form instead (e.g. `14 Sept 2026` — no leading zero on the day, `Sept` for September, never `Sep`). `service/dateFormat.js`'s `formatPublishDateForTutorial()` is the single deterministic ISO → human-facing converter and is the only place this conversion should happen; `scheduled_publish_date_consistency` compares the public field against that formatter's output, so a candidate with the right date but the wrong format (including still-ISO) FAILs.
8. **Publish-date is the tutorial list's primary sort key, not the title.** Dashboard tutorial lists (including Final Output) sort chronologically by Publish Date ascending; a missing Publish Date sorts after every dated record; the tutorial title is only ever a tie-breaker for equal (or equally missing) dates, never the primary key.
9. **Complete vs. Revamping lifecycle.** `"Complete"` means the human has approved the tutorial into Final Output (a `revampedOutputFile` on record) — this is an explicit, human-approved canonical value now, never inferred/detected from what other Final Output records happen to already use. `"Revamping"` means an unpublished tutorial currently has at least one revamp job (root or revision) sitting in a state that still needs human approval (`Queued` … `Ready for Review`, or `Needs Human Review`); a `Failed`/`Cancelled` job alone never keeps a tutorial marked `Revamping`. This effective status is derived at read time from job state (see `service/tutorialStatusService.js`) and is never written into `data/tutorials.json` for a merely-local, in-progress job — only an actual human-approved publish changes the committed dataset.
10. **A legacy tutorialId/URL/audit-filename naming mistake is never hardware or content authority.** A tutorial's `id`, `auditFile` path, and original source `url` are stable identifiers — they may contain an outdated or simply mistaken term (a wrong sensor model, an old product name) from earlier authoring, and are deliberately never renamed to "fix" that after the fact, since doing so would break audit references, job history, dashboard links, revision history, and existing external links. When a human explicitly resolves such a naming mistake (e.g. "the correct sensor is X, not what the ID says"), record the resolution as a `PROJECT_HARDWARE_DECISIONS` entry (§25) naming both the correct value and the legacy/incorrect one, so a matching deterministic check can fail a draft that lets the legacy identifier leak into the public body as if it were fact. Only human-facing, editable dataset fields that actually describe the tutorial's intended content (its `title`, structured hardware/technology/keyword lists) get corrected to the human-approved value — the identifier itself, and a still-live original source URL, are left exactly as they are.

## 29. Admin & SEO Metadata Refresh & Related-Tutorial Rule (Milestone 8)

1. **`Meta Tag Keywords` replaces `Meta Description`.** The Admin & SEO table (§23.3) no longer includes a `Meta Description` row — every new/revised draft uses `Meta Tag Keywords` instead, a concise comma-separated list, never both fields. Deterministic validation (`draftValidator.js`'s `meta_tag_keywords_present` / `no_obsolete_meta_description` checks) enforces this for newly generated/revised candidates only — it is never re-run against, and never fails, an already-immutable historical `service/jobs` artifact generated under the older standard.
2. **Keyword source priority.** Prefer deterministic existing metadata over inventing anything: (1) the tutorial's own structured `keywords` from `data/tutorials.json`, (2) its existing approved Tags/Categories where genuinely useful, (3) other human-approved context. Do not fabricate a long speculative SEO keyword list when no trustworthy source supports it. A keyword that a recorded `PROJECT_HARDWARE_DECISIONS` entry has since ruled out (e.g. a discontinued board no longer part of the architecture) is not reproduced merely because it appears in older dataset metadata — the more recent human-approved decision wins, same as everywhere else in this document.
3. **A Maker ESP32 tutorial's standard Related Tutorial is the Getting Started guide.** Include the canonical link (`service/approvedLinks.js`'s `MAKER_ESP32_GETTING_STARTED_URL`) in both the Admin & SEO "Related Tutorials" field and the public Related Tutorials/Community section, never marked `NEEDS VERIFICATION` and never substituted with a different URL. Other genuinely relevant, approved related tutorials may remain alongside it — this rule adds the standard entry, it does not remove or replace a useful existing one. This is a separate, standard *related-tutorial* placement from Prerequisites' own Getting Started link (§23.4) — the same URL serves both purposes; do not create a second, duplicate verification note for it.
