...
8. **Antigravity CLI is not on `PATH`, but is installed and its behavior is now verified** (2026-09-01) at `C:\Users\user\AppData\Local\Programs\Antigravity IDE\bin\antigravity-ide.cmd` (v1.107.0). Critically, its `chat` subcommand is **not** a headless request/response process: it launches/reuses the Electron IDE and returns without ever printing the model's response to stdout. A file-write test in `agent` mode confirmed Antigravity *can* write files to disk when instructed to in the prompt, so the integration model is "launch chat/agent, then watch a bridge-controlled output path" rather than "spawn and capture stdout" — see §7, §12, §18.1 for the full findings and the corrected design.
...
6. Attach `docs/CYTRON_TUTORIAL_AUTHORING_STANDARD.md` verbatim (a one-time, page-traceable, human-reviewed transcription of the Cytron Tutorial Template PDF — done in Milestone 3C-B; the PDF itself is never read at runtime).
...
**IMPLEMENTED (Milestone 6, 2026-09-08) — human feedback loop layered on top of Layer B, no new job type.** A `Request Changes` action on an eligible draft (`Ready for Review` / `Needs Human Review`, unpublished) creates a brand-new job — still `type: 'revamp'`, still running through the exact same `Queued → Preparing Context → Writing → Validating → { Ready for Review | Needs Human Review }` state machine as a root job — that additionally carries `parentJobId`, `rootJobId`, `revisionNumber`, and `reviewFeedback` (see `service/jobStore.js`'s `createRevisionJob`/`deriveRevisionMeta`/`listRevisions`). The parent job is never mutated, never re-run, and never deleted — it stays a permanent, byte-identical rung in an immutable revision ladder (Job A → Job B with `parentJobId=A` → Job C with `parentJobId=B`, all sharing `rootJobId=A`). A job created before Milestone 6 has none of these fields; they are derived at read time as `revisionNumber=1, rootJobId=<own id>, parentJobId=null` rather than migrating any existing `job.json`. See `docs/TUTORIAL_REVAMP_AGENT_MILESTONE_6_REVISIONS.md` for the full design.
...
**IMPLEMENTED (Milestone 5, 2026-09-03):** `POST /api/revamp/:jobId/publish` added — human-approved Final Output publish (candidate → `revamped-tutorials/`, dataset update, validate, commit, push). See `docs/TUTORIAL_REVAMP_AGENT_MILESTONE_5_APPROVAL_PUBLISH.md`.

**IMPLEMENTED (Milestone 6, 2026-09-08):** two endpoints added, both authenticated, both resolving every filesystem path exclusively from the persisted parent job (never from the request body): `POST /api/revamp/:jobId/revise` (body: `{feedback}` only — creates a new revision job from an eligible, unpublished parent + human review feedback; see `service/revisionManager.js`) and `GET /api/revamp/:jobId/revisions` (safe revision-history summary — `jobId`/`revisionNumber`/`parentJobId`/`state`/`createdAt`/`validationSummary` only, no filesystem paths — for the whole chain the given job belongs to). Milestone 5's publish endpoint required zero changes: a revision job is `type: 'revamp'` like any other, so `tutorialPublisher.js` already resolves its candidate correctly by `jobId` alone.
...
12. **`references/` and `tmp/` are read-only inputs** to the bridge; nothing in this design ever writes into them.
...
3. **How is the original tutorial URL retrieved server-side? — RESOLVED (Milestone 3C-A, 2026-09-02).** Confirmed missing in Milestone 3B; implemented and proven in Milestone 3C-A. `service/originalTutorialSource.js` fetches `tutorial.url` (never browser-supplied, never hardcoded) through a strict, per-redirect-hop-revalidated policy (`service/urlPolicy.js`: HTTPS only, fixed hostname allow-list derived from the dataset — currently just `my.cytron.io` — no credentials, no localhost, no private/loopback IPs via DNS-rebinding-aware checks, ≤3 redirects, 20s timeout, 5MB cap), and `service/htmlExtractor.js` (a new `cheerio` dependency — the project's first non-zero-dependency code, added deliberately for this) deterministically extracts the article body (`#blog-description`/`.post_content`) into Markdown. One real end-to-end run against `esp32-digital-clock` succeeded (200, 58KB → 6.7KB extracted, 19 headings, table/code/lists/links all preserved) and `promptBuilder.js`'s dry run for that tutorial dropped from 2 missing required sources to exactly 1 (only the Template PDF remains, per deliberate scope). Full detail, including an honest note that the live page's current content no longer matches the audit's description of the "original": `docs/TUTORIAL_REVAMP_AGENT_MILESTONE_3C-A.md`.
4. **PDF reference caching — RESOLVED (Milestone 3C-B, 2026-09-02), by deliberately NOT caching the PDF at runtime.** Confirmed in Milestone 3B that `tmp/pdf-template-review/` and `tmp/maker-esp32-datasheet/` contain only rendered page **PNG images**, not text — there is no deterministic runtime path to the PDF's content, and none was built. Instead, the PDF (`references/Cytron Tutorial Template 290826.pdf` — a clean, fully text-selectable export, no OCR needed) was reviewed **once**, by Claude Code directly, into a new canonical file: `docs/CYTRON_TUTORIAL_AUTHORING_STANDARD.md`, with page-level traceability back to the PDF and an explicit section-by-section comparison against `AGENTS.md` (no genuine conflicts found; three internal PDF inconsistencies recorded, not silently resolved). **That Markdown file, not the PDF, is now the `REQUIRED` runtime source** `promptBuilder.js` reads — the PDF itself is recorded only as non-required provenance (`fs.existsSync` check only, never opened). Separately confirmed: `references/Maker ESP32 Datasheet Rev1.0 June2026.pdf` was never actually part of `AGENTS.md`'s required hierarchy at all — hierarchy item 3 names the plain-Markdown `Maker ESP32 AI Coding Pack` at `E:\Cytron-AI-Coding-Pack\cytron-ai-coding-pack-maker-esp32\` instead, which was already reliably readable with zero new dependencies. Full detail: `docs/TUTORIAL_REVAMP_AGENT_MILESTONE_3C-B.md`.
...
6. **Extending `tutorials.json`'s `revampStatus` enum** with `Ready for Review` (§9) touches a schema the user/team may have downstream expectations about (e.g. anything reading the CSV export, `tutorial_validation_export.csv`) — confirm no other consumer assumes the current fixed enum. *(Milestone 5 note: the actual Final Output promotion path never introduces a new enum value — it detects and reuses whatever `revampStatus` value the existing Final Output records already share, via `tutorialsJsonRecordEditor.detectEstablishedRevampStatus()`.)*
...
8. **Wiring the proven real writer into the normal dashboard flow — RESOLVED (Milestone 4, 2026-09-02).** The Milestone 2 StubWriter behind `POST /api/revamp/start` was replaced with the real, already-proven Milestone 3 writer pipeline (`tutorialWriterPilot.js`'s `runWriterForJob()`, shared with the supervised pilot path — no duplicated writer logic). A real, confirmed bug was found and fixed in the process: the pilot's preflight checks (non-empty instructions, literal "educational"/"not certified" wording, mandatory Maker ESP32 targeting) were specific to the one safety-sensitive pilot project and would have blocked `agy` from ever being called for almost every other tutorial — verified with an isolated test before removing them as hard preconditions. Output retrieval, a "View Final Tutorial" draft-mode page, browser-refresh recovery, and cancellation were all added; publishing/promotion (writing `revamped-tutorials/`, patching `tutorials.json`) remained entirely unbuilt at that point — see item 9 below. Full detail: `docs/TUTORIAL_REVAMP_AGENT_MILESTONE_4_DASHBOARD_WRITER.md`.
9. **Human-approved publish/promotion path — RESOLVED (Milestone 5).** `POST /api/revamp/:jobId/publish` (via `service/tutorialPublisher.js`) now performs the step item 8 deferred: copy the approved candidate to `revamped-tutorials/<id>.md` byte-for-byte, patch `data/tutorials.json` via targeted line-level surgery (never a full-file rewrite), re-run `scripts/validate-data.js`'s checks (now exported as a reusable function) as a hard gate with automatic rollback on any failure, then `git add`/`commit`/`push origin main` using only fixed, server-derived arguments (`shell:false`, plain argv, no browser-suppliable path/command/message). Two things this milestone deliberately did NOT do: extend the `revampStatus` enum (it detects and reuses the existing convention — see item 6's amendment) or allow overwriting an already-published Final Output (refused unconditionally; a replace workflow is explicitly future work). Full detail: `docs/TUTORIAL_REVAMP_AGENT_MILESTONE_5_APPROVAL_PUBLISH.md`.
10. **CMS HTML Export for approved Final Output — ADDED (Milestone 9, static-only, no bridge involved).** `final-output.html?id=<id>` (static mode only, never draft/`?jobId=` mode) gained **Copy CMS HTML** / **Preview CMS HTML** actions backed by a new, dependency-free, DOM-free module `js/tutorial-html-export.js`. It reads the same already-approved `revampedOutputFile` Markdown the page already renders, extracts only the Introduction → end-of-Troubleshooting range (heading-structure based, not string search; a missing boundary is a hard error, never a guessed range), strips the Sample Code program's fenced block(s) down to a single `<!-- INSERT GITHUB GIST EMBED HERE -->` placeholder (the human pastes their own Gist embed in the CMS), and converts the remainder to a clean semantic HTML fragment (no `<html>/<head>/<body>/<style>/<script>`) with inline `text-align: justify;` on prose paragraphs and inline code rendered as `<em>` rather than `<code>` — both because the CMS editor never loads this dashboard's stylesheet. Entirely read-only and static: no write path touches `data/tutorials.json`, `revamped-tutorials/`, or git. Full detail: `docs/TUTORIAL_REVAMP_AGENT_MILESTONE_9_CMS_HTML_EXPORT.md`.
